...
class CredentialServices {
  getById(id) {
    return fetch(
      `${baseUrl}/Credentials/${id}?access_token=${localStorage.getItem(
        "token"
      )}`,
      {
        method: "GET",
  ...
  login(credentials) {
    return fetch(`${baseUrl}/Credentials/login`, {
      ...
      if (!res.ok) {
        throw new Error(
          "No se pudo iniciar sesión, verifique las credenciales ingresadas"
        );
      } else {
        return res.json();
      }
  ...
  resetPassword(newPassword, accessToken) {
    return fetch(
      `${baseUrl}/Credentials/reset-password?access_token=${accessToken}`,
      {
        method: "POST",
        headers: {
          Accept: "application/json",
  ...
  changePassword(passwordValues) {
    return fetch(
      `${baseUrl}/Credentials/change-password?access_token=${localStorage.getItem(
        "token"
      )}`,
      {
        method: "POST",
