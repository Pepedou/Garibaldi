...
class ArtistCard extends Component {
  ...
  handleCategoryValidation(data, position, property, propertyName) {
    ...
    if (propertyName === "category") {
      ...
      if (data === "") {
        editedArtistCopy.detail[
          propertyName
        ].value = this.props.currentArtist.detail[propertyName].value;
      } else {
  ...
  getPhoto(editedArtist) {
    if (Object.getOwnPropertyNames(editedArtist).length > 0) {
      if (
        editedArtist.detail.photo.value === "" ||
        !editedArtist.detail.photo.value
      ) {
    ...
    return (
      <div className="ArtistCard">
        ...
        <CardComponent
          ...
          cardDescription={
            <CardDescription
              artistCardInformation={editedArtist}
...
export default withRouter(
  connect(null, { configExportForArtists: loadTemplateConfigForArtists })(
    ArtistCard
  )
);
