...
class ArtCard extends Component {
  ...
  handleCategoryValidation(data, position, property, propertyName) {
    ...
    if (propertyName === "category") {
      ...
      if (data === "") {
        currentCategoriesCopy = [
          ...currentCategoriesCopy.slice(0, position),
          ...currentCategoriesCopy.slice(position + 1)
        ];
      } else {
      ...
      if (data === "") {
        editedArtCopy.detail[propertyName].value = this.props.currentArt.detail[
          propertyName
        ].value;
      } else {
  ...
  handleOnUpdateInput(data) {
    ...
    if (
      !this.state.dataSource.find(
        item => item.text.toUpperCase() === data.toUpperCase()
      )
    ) {
    ...
    let newArt = {
      ...
      images:
        sourceImage !== ""
          ...
          : this.state.editedArt.detail.images.value
    };
  ...
  render() {
    let {
      showFullImageOverlayRecieved,
      showDropZoneOverlayRecieved,
      sourceImage
    } = this.props;
    ...
    return (
      <div className="ArtCard">
        <div className="imageHeaderBar row">
          <div
            className="editImageBtn col-xs-6 col-md-6"
            onClick={() => showDropZoneOverlayRecieved(true)}
          >
          ...
          <div
            className="FullImage-button col-xs-6 col-md-6"
            onClick={() => showFullImageOverlayRecieved(true)}
          >
            <SvgIcon style={editBtnStyle}>
        ...
        <CardComponent
          ...
          cardImage={
            sourceImage !== ""
              ? sourceImage
              : editedArt.detail.images.value.standard
          }
          cardTitle={null}
          cardSubtitle={null}
          cardDescription={
            <CardDescription
              artCardInformation={editedArt}
              onTouchTap={this.handleAddCategory.bind(this)}
              handleCategoryValidation={this.handleCategoryValidation.bind(
                this
              )}
            />
          }
          cardActions={
            <CardActions
              ...
              handlePDF={this.handlePDF.bind(this)}
            />
          }
        />
      </div>
...
export default withRouter(
  connect(null, { configExportForArtPieces: loadTemplateConfigForArtPieces })(
    ArtCard
  )
);
