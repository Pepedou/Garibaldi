  ...
  render() {
    ...
    return showPdfPreviewOverlay ? (
      <div className="Overlay PdfPreviewOverlay">
        <a
          className="Closebtn noPrint"
          onClick={() => this.toggleOverlay(showPdfPreviewOverlayRecieved)}
        >
          &times;
        </a>
        <div className="overlayContent">
          <center>
            {exportFile.EXPFILE1.pages.map((value, key) => {
              let pageBgClassName = `pdfPage ${
                exportTemplates.allTemplates[exportFile.EXPFILE1.template]
                  .backgroundPosition
              }`;
              let bgStyle = {
                backgroundImage: `url(${
              ...
              let pageLogoClassName = `pdfLogo ${
                exportTemplates.allTemplates[exportFile.EXPFILE1.template]
                  .logoPosition
              }`;
              return (
                <div className={pageBgClassName} style={bgStyle} key={key}>
                  {exportTemplates.allTemplates[exportFile.EXPFILE1.template]
                    .logo !== "" ? (
                    <div className="logoWrapper row">
                      <img
                        alt=""
                        src={
                          exportTemplates.allTemplates[
                            exportFile.EXPFILE1.template
                          ].logo
                        }
                  ...
                  <div className="pageContent row" key={key}>
                    <div className="col-xs-12 col-md-12">
                      {exportPages[value].withImage ? (
                        <img
                          alt=""
                          src={exportPages[value].image}
                          className="itemImage"
                        />
                      ) : null}
                      <div className="categoriesWrapper">
                        {exportPages[value].categories.map((category, key) => (
                          <div key={key} className="categoryRow">
                            <div className="categoryLabel">
                              {exportCategories[category].label}:
                            </div>
                            <div className="categoryValue">
                              {exportCategories[category].value}
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  {exportTemplates.allTemplates[exportFile.EXPFILE1.template]
                    .lineColor !== "" ? (
                    <div className="membrete" style={membreteStyle} />
                  ) : null}
                </div>
              );
            })}
