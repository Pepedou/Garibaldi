...
export default class SqueezeBox extends Component {
    ...
    handleSlideBoxClicked(artistClicked) {
        this.setState({selectedArtist: artistClicked});
    }
    ...
    nextSides() {
        ...
        if ($('.no-csstransitions').length > 0) 
            this.updateSlider('next', actual, slider, following);
        }
    ...
    prevSides() {
        ...
        if ($('.no-csstransitions').length > 0) 
            this.updateSlider('prev', actual, slider);
        }
    ...
    // TODO: Implement event. if on desktop - set a width for the projectsSlider
    // element $(window).on('resize', function(){     //on resize - update
    // projectsSlider width and translate value     if( !resizing ) {
    // (!window.requestAnimationFrame) ? setSliderContainer() :
    // window.requestAnimationFrame(setSliderContainer);         resizing = true;
    //  } });
        ...
        return (
            <div className="squeezeBox">
                <div className="cd-projects-wrapper">
                    <SqueezeSlider
                        elements={this.state.artists}
                        onPrevSidesClicked={this.prevSides}
                        onNextSidesClicked={this.nextSides}
                        onBoxClick={this.handleSlideBoxClicked}/>
                    <SqueezeNavControls
                        handlePreviousClick={this.prevSides}
                        handleNextClick={this.nextSides}/>
                </div>
                <SqueezeFrontCard
                    artist={selectedArtist}
                    onClickHandler={this.handleDetailCardClose}/>
            </div>
        );
    }
}