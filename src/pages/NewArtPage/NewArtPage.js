...
class NewArtPage extends Component {
    ...
    ArtistServices.getAll()
      .then(function(response) {
        ...
        if (currentUser.ownerType === UserTypes.ARTISTA) {
          ...
          let newInputList = [
            ...inputFieldsCopy.slice(0, currentFieldIndex),
            ...inputFieldsCopy.slice(currentFieldIndex + 1)
          ];
  ...
  handleOnClick(
    event,
    { clearAllNotifications, addNotification, loading, currentUser, router }
  ) {
    loading(true);
            ...
            <div className="col-xs-12 col-md-4 NewArtForm">
              {this.state.inputFields.map((item, key) => (
                <InputFieldComponent
                  ...
                  onUpdateInput={value =>
                    this.handleOnUpdateInput(value, item.id)
                  }
                />
              ))}
            </div>
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(NewArtPage)
);
